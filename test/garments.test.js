...
describe('As part of the sql refresh workshop', () => {
	...
	it('you should be able to change a given Male garment to a Unisex garment', async () => {
		...
		const result = await db.one('update garment set gender = $1 where gender = $2 and description = $3 returning $4',['Unisex', 'Male', 'Red hooded jacket', 'sucess'])
	...
	it('you should be able to add 2 Male & 3 Female garments', async () => {
		...
		const garment_add = 'insert into garment (description, img, season, gender, price) values ($1,$2,$3,$4,$5);'
		db.none(garment_add, ['Red Shirt', 'womans-128x128-455144.png', 'All Seasons', 'Male', '199.99']);
		db.none(garment_add, ['Red hooded jacket','hoodie-128x128-455122.png', 'Winter', 'Male', '299.99']);
		db.none(garment_add, ['Orange dress(formal)', 'frock-128x128-455120.png', 'Summer', 'Female', '249.99']);
		db.none(garment_add, ['Short Skirt(Lime)', 'skirt-128x128-455130.png', 'Summer', 'Female', '199.99']);
		db.none(garment_add, ['Golf t-shirt(Red)', 'womans-128x128-455139.png', 'All Seasons', 'Female', '79.24']);
	...
	it('you should be group garments by gender and count them', async () => {
		...
		const garmentsGrouped = await db.many(
		  `select count(*), gender from garment group by gender order by gender`,
		);
		// write your code above this line
	...
	it('you should be able to remove all the Unisex garments', async () => {
		...
		// and below this line for this function will
		const delete_garment=db.none('delete from garment where gender = $1',['Unisex'],)
		// write your code above this line
